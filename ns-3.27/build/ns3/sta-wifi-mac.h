...
class CompareBeaconInfo
{
public:
    bool operator() (BeaconInfo left, BeaconInfo right)
    {
        return left.m_snr < right.m_snr;
    }
};
...
public:
  ...
  std::priority_queue<BeaconInfo, std::vector<BeaconInfo>,  CompareBeaconInfo> bssidQueue;

  std::vector<BeaconInfo> backupBssids;

  std::map<Mac48Address, bool> queuedBssids;

  Mac48Address m_lastRequestedBssid;
  ...
  /**
    *  Start the scanning process to find the strongest probe response
    */
  void StartScanningHandover (void);

  void addToBssidQueue(BeaconInfo);

  BeaconInfo popBssidQueue();

  void clearBssidQueue();
...
/**
  * Continue scanning process for handover or terminate if no further channels to scan
  */
void ScanningHandoverTimeout (void);


 virtual void DoInitialize (void);

 void SelectAPForAssociation(void);

 void PreSendAssociationRequest (void);

  enum MacState m_state;
  Time m_probeRequestTimeout;
  Time m_assocRequestTimeout;
  Time m_scanningTimeout;
  Time m_scanningHOTimeout;
  ...
  std::vector<uint8_t> m_candidateChannels; /// used when scanning
  BeaconInfo m_bestBeaconObserved; /// used when scanning
  BeaconInfo m_bestProbeObserved; /// used when scanning for Handover
