...
public:
  ...
  */
  ...
  void SetPath (std::vector<uint32_t>);

  void SetIsTimeStamp (bool);
...
private:
  ...
  std::vector<uint32_t>    m_path;       //!< The packet of which copies should be sent. If this is explicitly specified, the variable m_pktsize below has no role. 
  ...
  bool            m_istimestamp;  //!< Should a timestamp be added to each packet
