...
vector<vector<double> > ext_locations = {
  {6, 3},
{8, 6},
{6, 3},
{7, 7},
...
//4,6,12,14,16,18,20,22,24,26,29,31,34,35,38,40,49
...
class Experiment{
  ...
  void changeExtLocation(bool nearer){
    ...
    cout<<"x: "<<new_location.first<<", y: "<<new_location.second<<endl;
  ...
  void peekChannels(){
    cout<<"channel ap backhaul: "<<GetSpectrumWifiPhyPtr(apDeviceBackhaul.Get(0))->GetChannelNumber()<<endl;
    cout<<"channel ext backhaul: "<<GetSpectrumWifiPhyPtr(extDeviceBackhaul.Get(0))->GetChannelNumber()<<endl;
    cout<<"channel ap fronthaul: "<<GetSpectrumWifiPhyPtr(apDeviceFronthaul.Get(0))->GetChannelNumber()<<endl;
    cout<<"channel ext fronthaul: "<<GetSpectrumWifiPhyPtr(extDeviceFronthaul.Get(0))->GetChannelNumber()<<endl;
    for(int i=0;i<nStas;i++){
      cout<<"channel sta "<<i<<": "<<GetSpectrumWifiPhyPtr(staDevices.Get(i))->GetChannelNumber()<<endl;
    }
  }
  ...
  void channelChangeHandler(){
    //cout<<"ch_bh_to_change: "<<ch_bh_to_change<<" ch_fh_to_change: "<<ch_fh_to_change<<endl;
    ...
    for(int i=0;i<nStas;i++){
      if(ch_fh_to_change != currentChannels[4+i]){
        cout<<"change "<<i<<endl;
        cout<<"ch_fh_to_change: "<<ch_fh_to_change<<" currentChannel: "<<currentChannels[4+i]<<endl;
...
void write_to_matlab(){
      ...
      while(1){
          ...
          cout<<"json_to_send_string: "<<json_to_send_string<<endl;
          n = write(sockfd,(json_to_send_string+ "\r").c_str(), json_to_send_string.size() + 1);
          
          if (n < 0) 
               error("ERROR writing to socket");
          cout<<"wrote to socket"<<endl;
          cout<<"TIMESTAMP: "<<json_to_send.at("timestamp").get<double>()<<endl;
  ...
  void read_from_matlab(){
      ...
      //else cout<<"successful binding"<<endl;
      listen(sockfd,5);
      ...
      //else cout<<"successful accept"<<endl;
      while(1){
          if(iter>1){
            cout<<"iter>1"<<endl;
            cout<<is_paused<<endl;
          ...
          std::unique_lock<std::mutex> lk_pause(m_pause);
          while(!is_paused){
              cout<<"waiting for simulation to pause"<<endl;
              cv_pause.wait(lk_pause);
              if (!is_paused)
                  std::cout << "Spurious wake up!\n";
              cout<<"simulation paused... going ahead"<<endl;
          }
          cout<<"waiting for matlab"<<endl;
          n = read(newsockfd,buffer,255);
          if (n < 0) error("ERROR reading from socket");
          //else cout<<"success reading from socket"<<endl;
          //printf("%s\n",buffer);

          command_read = buffer;
          trim(command_read);
          //if(arr.size() == 0) exit(1);
          cout<<"Message from Matlab: "<<endl;
          cout<<command_read<<endl;
